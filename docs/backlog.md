...
| 优先级 | 项 | 说明 |
|---|---|---|
| P1 | **桌面外壳** | 本地 Web 控制台**只读状态面板已完成**（`internal/webui`，node `-web`：身份/角色/出口/负载/已连出口/地区/信任范围/运行时长，自刷新）。**剩：运行时控制**（切换出口/开关出口/小网管理）与系统托盘。 |
...
### Phase 4 — 面向用户（← 当前阶段）
13. 桌面外壳：只读状态面板 ✅（node `-web`）。**剩：运行时控制**（切换出口/开关出口/小网管理，需节点支持运行时重配）+ 图形化首启动信任向导 + 系统托盘。
