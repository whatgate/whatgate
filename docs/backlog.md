...
| 优先级 | 项 | 说明 |
|---|---|---|
| P1 | **桌面外壳** | 本地 Web 控制台：状态面板 ✅ + **运行时切换出口地区** ✅（`SwitchableDialer` + `/api/switch`，浏览器里换地区无需重启）。**剩：运行时开关出口 / 小网管理 / 首启动向导 / 系统托盘。** |
...
### Phase 4 — 面向用户（← 当前阶段）
13. 桌面外壳：状态面板 ✅ + 运行时切换出口地区 ✅（`/api/switch`）。**剩：运行时开关出口 / 小网管理 / 图形化首启动信任向导 / 系统托盘。**
