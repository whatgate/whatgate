...
| **C1.2 撤销 checkpoint + 陈旧兜底** ✅**已完成** | `membership.RevocationCheckpoint`（根签+单调版本反回滚+thisUpdate/nextUpdate/maxStaleness 双层新鲜度+revokedSubjects/Issuers+revEpoch）；`Revokes`/`Stale`；离线根 `-emit-revocation` CLI。commit 4b61e7e，8 测试全绿+真机。运行时消费（选路拒撤销出口/陈旧降级）接 C1.4 | 撤销 subject/issuer→`Revokes`；超陈旧上限→`Stale` 触发仅应急；回滚/硬过期/篡改→拒 ✅ |
| **C1.3 私有认证 DHT 引导 + 入站门控** ✅**库层完成**（接线 C1.4） | **段1**（fd184ad）：`MemberSet` + `memberGater`（ConnectionGater）+ `PrivateDHT`（`/whatgate/kad` 前缀 + RoutingTableFilter + ModeServer）+ `WithMemberGater`/`StartPrivateDHT`。**段2**（e506211）：`/whatgate/member-auth/1.0.0` 连接后证书握手（nonce 签名 + 链验根 + subject==peer）`SetMemberCredential`/`VerifyPeerMembership`。正式引入 kad-dht v0.42.1。5 测试全绿(真实 libp2p) | 陌生人连接服务端 NotConnected/不进 RT ✅；两成员组私有 DHT ✅；证书 subject 不符/错根/越权 → 拒 ✅ |
| **C1.4 节点记录两层发现 + 四状态合并 + 拨号策略** 🟡**进行中** | **C1.4a**（102bcdb）：`membership.NodeRecord`（自签+generation 反回滚+角色⊆证书）+ `/whatgate/noderecord/1.0.0` 认证拉取（`SetNodeRecord`/`FetchNodeRecord`，含连通性 nonce）。**C1.4b**（e200d35）：`CapabilityCID(role,region,epoch)` + `PrivateDHT.Advertise`/`FindCandidates`（DHT provider 粗索引）。两层齐（FindCandidates→FetchNodeRecord）。**C1.4c**（3a8008f）：`SafeDialAddrs` 拨号 SSRF 过滤（复用 F1）折入 `FetchNodeRecord`。**C1.4d**（c2ad041）：`EquivocationGuard`（同 generation 异 payload→隔离）。**C1.4e**（f5c4303）：`Node.ResolveExits` 组合 pipeline（FindCandidates→FetchNodeRecord→撤销丢弃→equivocation 隔离→`VerifiedExit` 四状态标注，DHT 永不给 recommended，陈旧 checkpoint→Eligible=false 应急）；真实 DHT 端到端测。**C1.4f 双轨合并**（3c70e88）：`routing.MergeExits` 协调器目录+DHT 出口去重合并，DHT-only=stranger，喂 RankExits（保守 scope 排除 DHT-only）。**待办 C1.4f 尾（cmd/node 接线，需双机实测）**：节点持凭据/服务记录/Advertise + discoverExit 回退 ResolveExits→MergeExits + 门控与 relay/协调器调和 | 重放/越权/错签名者/SSRF/equivocation/撤销 → 拒 ✅；四状态合并 ✅；cmd 主流程接线 → 待（双机验证）|
